...
import automata.com.keyless.data.WatchWallet;
import automata.com.keyless.fragments.FragmentPrice;
...
import automata.com.keyless.services.WalletGenService;
import automata.com.keyless.utils.AddressNameConverter;
import automata.com.keyless.utils.Dialogs;
import automata.com.keyless.utils.ExchangeCalculator;
import automata.com.keyless.utils.ExternalStorageHandler;
import automata.com.keyless.utils.OwnWalletUtils;
import automata.com.keyless.utils.Settings;
import automata.com.keyless.utils.WalletStorage;
...
public class MainActivity extends SecureAppCompatActivity implements NetworkUpdateListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (preferences.getLong("APP_INSTALLED", 0) == 0) {
            Intent intro = new Intent(this, automata.com.keyless.activities.AppIntroActivity.class);
            startActivityForResult(intro, automata.com.keyless.activities.AppIntroActivity.REQUEST_CODE);
        }
    ...
    public void onActivityResult(int requestCode, int resultCode, final Intent data) {
        ...
        if (requestCode == automata.com.keyless.activities.QRScanActivity.REQUEST_CODE) {
            if (resultCode == RESULT_OK) {
                byte type = data.getByteExtra("TYPE", automata.com.keyless.activities.QRScanActivity.SCAN_ONLY);
                if (type == automata.com.keyless.activities.QRScanActivity.SCAN_ONLY) {
                    if (data.getStringExtra("ADDRESS").length() != 42 || !data.getStringExtra("ADDRESS").startsWith("0x")) {
                        snackError("Invalid Ethereum address!");
                        return;
                    }
                    Intent watch = new Intent(this, automata.com.keyless.activities.AddressDetailActivity.class);
                    watch.putExtra("ADDRESS", data.getStringExtra("ADDRESS"));
                    startActivity(watch);
                } else if (type == automata.com.keyless.activities.QRScanActivity.ADD_TO_WALLETS) {
                    if (data.getStringExtra("ADDRESS").length() != 42 || !data.getStringExtra("ADDRESS").startsWith("0x")) {
                        snackError("Invalid Ethereum address!");
                        return;
                    }
                ...
                } else if (type == automata.com.keyless.activities.QRScanActivity.REQUEST_PAYMENT) {
                    if (WalletStorage.getInstance(this).getFullOnly().size() == 0) {
                        Dialogs.noFullWallet(this);
                    } else {
                        Intent watch = new Intent(this, automata.com.keyless.activities.SendActivity.class);
                        watch.putExtra("TO_ADDRESS", data.getStringExtra("ADDRESS"));
                        watch.putExtra("AMOUNT", data.getStringExtra("AMOUNT"));
                        startActivity(watch);
                    }
                } else if (type == automata.com.keyless.activities.QRScanActivity.PRIVATE_KEY) {
        ...
        } else if (requestCode == automata.com.keyless.activities.WalletGenActivity.REQUEST_CODE) {
            if (resultCode == RESULT_OK) {
        ...
        } else if (requestCode == automata.com.keyless.activities.SendActivity.REQUEST_CODE) {
            if (resultCode == RESULT_OK) {
        ...
        } else if (requestCode == automata.com.keyless.activities.AppIntroActivity.REQUEST_CODE) {
            if (resultCode != RESULT_OK) {
                finish();
            } else {
                SharedPreferences.Editor editor = preferences.edit();
        ...
        } else if (requestCode == automata.com.keyless.activities.SettingsActivity.REQUEST_CODE) {
    ...
    public void importPrivateKey(String privatekey) {
        Intent genI = new Intent(this, automata.com.keyless.activities.WalletGenActivity.class);
        genI.putExtra("PRIVATE_KEY", privatekey);
        startActivityForResult(genI, automata.com.keyless.activities.WalletGenActivity.REQUEST_CODE);
    }
    ...
    private void selectItem(int position) {
        switch (position) {
            ...
            case 2: {
                Intent settings = new Intent(this, automata.com.keyless.activities.SettingsActivity.class);
                startActivityForResult(settings, automata.com.keyless.activities.SettingsActivity.REQUEST_CODE);
                break;
            }
            case 3: {
